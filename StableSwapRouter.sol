...
contract StableSwapRouter {
    ...
    function calculateConvert(
        ...
        uint256 fromPoolLength = fromPool.getTokenLength();
    ...
    function convert(
        ...
        uint256 fromPoolLength = fromPool.getTokenLength();
        uint256 toPoolLength = toPool.getTokenLength();
    ...
    function calculateRemoveBaseLiquidityOneToken(
        ...
        IERC20 baseToken = IERC20(getLpToken(basePool));
        uint8 baseTokenIndex = pool.getTokenIndex(address(baseToken));
        uint256 _base_tokens = pool.calculateRemoveLiquidityOneToken(address(this), _token_amount, baseTokenIndex);
    ...
    function calculateSwapFromBase(
        ...
        uint256[] memory base_amounts = new uint256[](basePool.getTokenLength());
        base_amounts[tokenIndexFrom] = dx;
        uint256 baseLpAmount = basePool.calculateTokenAmount(address(this), base_amounts, true);
        if (baseTokenIndex == tokenIndexTo) {
            return baseLpAmount;
        }
